...
class Player(Character):
	...
	def move(self, collision_zone, movement_cost_area, bike=None, mushroom=None):
		...
		if not hit_wall:
			if self.space_up and self.bullet_interval == 0 :
				self.space_up = False
				if len(self.inventory) < 10: #fire at most 10
		...
		if self.x > config.window_height-self.width:
			self.x -= self.vel
		elif self.x < 0:
			self.x += self.vel
		elif self.y < 0:
	...
	def check_kill(self, bullet, enemy):
		...
		if bullet.x + bullet.width >= -sq + enemy.x and bullet.x <= enemy.x + enemy.width + sq:
			if bullet.y + bullet.height >= -sq +  enemy.y and bullet.y <= enemy.y + enemy.height + sq:
